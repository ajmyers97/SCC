//Module:	CIS023_S2018_Lab6a Allen Myers.cpp
...
#include "stdafx.h"
#include "CIS023_S2018_Lab6a Allen Myers.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_CIS023_S2018_LAB6AALLENMYERS, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CIS023_S2018_LAB6AALLENMYERS));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CIS023_S2018_LAB6AALLENMYERS));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_CIS023_S2018_LAB6AALLENMYERS);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        {
			...
			case BN_CLICKED:										//button clicked
				if(LOWORD(lParam) == (WORD)hWnd_btnHextoInt)		//top convert button
				{							
					TCHAR szInput[100];								//user input
					int iConverted;									//return valeu from convert
					TCHAR szOutput[100];							//output to window

					GetWindowText(									//get etxt form a window
					hWnd_Input_1,									//handle to edit control
					szInput,										//destination TCHAR
					100);											//size of TCHAR
					
					HexClass HexObject;								//create object

					iConverted =
						HexObject.Convert(szInput);					//convert hex to integer

					_stprintf_s(									//to convert integer to TCHAR
						szOutput,									//destination TCHAR
						TEXT("%i"),									//pattern
						iConverted);								//integer


					SetWindowText(									//send TCHAR to window
						hWnd_Output_1,								//target static control
						szOutput);									//TCHAR



				}
				if (LOWORD(lParam) == (WORD)hWnd_btnInttoHex)
				{
					TCHAR szInput[100];								//user input
					int iInput;										//user input converted to integer
					TCHAR szOutput[100];							//output to window

					GetWindowText(									//get etxt form a window
						hWnd_Input_2,								//handle to edit control
						szInput,									//destination TCHAR
						100);										//size of TCHAR
					iInput = _tstoi(szInput);						//convert TCHAR input to integer

					HexClass HexObject;								//create object

					_tcscpy_s(										//copy converted value to output
						szOutput,									//output TCHAR
						100,										//size of return TCHAR
						HexObject.Convert(iInput)					//convert int to hex
					);

					SetWindowText(									//send TCHAR to window
						hWnd_Output_2,								//target static control
						szOutput);									//TCHAR
	...
	case WM_CREATE:
	{
		...
		SetWindowText(hWnd, TEXT("CIS023_S2018_Lab6a Mark Berrett"));
	}
	break;
