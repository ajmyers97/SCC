//Module:	CIS023_S2018_Lab11b.cpp
//Project:	CIS023_S2018_Lab11b
//Author:	Allen Myers
//Date:		April 25th, 2018
//Purpose:	Traverse a binary tree.
//			Perform a search for data in a binary tree.
//			Display results of a binary tree search.
//

#include "stdafx.h"
#include "CIS023_S2018_Lab11b.h"
...
char szFileName[] = "CIS023_S2018_Lab11b_Data.txt";			// file that contains data for binary tree
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_CIS023_S2018_LAB11B, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CIS023_S2018_LAB11B));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_CIS023_S2018_LAB11B));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_CIS023_S2018_LAB11B);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case BN_CLICKED:
			...
			{
				TCHAR szInput[TCHAR_SIZE];						//from edit control
				TCHAR szOutput[TCHAR_SIZE];						//from return string

				string strInput;								//converted from szInput
				string strResult;								//returned from btree->Search();
				size_t pRet;									//return value from mbstowcs_s
				//get user input from edit control
				SendMessage(hWnd_Input, WM_GETTEXT, TCHAR_SIZE, LPARAM(szInput));
				//convert input to string
				wstring temp(szInput);							//convert TCHAR to string
				string str(temp.begin(), temp.end());				//convert stream to string
				strInput = str;									//copy to local string
				
				if (!strInput.empty())							//if input
				{
					strResult = bTree->Search(strInput);			//search for the user input in the bTree

					//display return string into static control
					mbstowcs_s(										//convert string to tchar
						&pRet,										//return value
						szOutput,									//target TCHARE
						TCHAR_SIZE,									//size of TCHAR
						strResult.c_str(),							//source string	
						TCHAR_SIZE									//max # chars to copy
					);
					SetWindowText(hWnd_Output, szOutput);			//display text back to window
				}
			}

			if (LOWORD(lParam) == (WORD)hWnd_Close)				//close button
				DestroyWindow(hWnd);							//send WM_DESTROY message to close the window
			break;
		case IDM_ABOUT:
